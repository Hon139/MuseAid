...
# Example usage: Add entry with id=1, empty data, no code
if __name__ == "__main__":
    # Example entry as requested
    example_id = 2
    example_data = {}      # Empty JSON data
    example_code = None    # No code
    
    print("Adding example entry...")
    add_entry(example_id, example_data, example_code)
    
    # Example: Check if an entry exists
    print(f"\nChecking if entry with id {example_id} exists...")
    entry_exists(example_id)
    
    # Example: Get entry by ID
    print(f"\nGetting entry with id {example_id}...")
    retrieved_entry = get_entry_by_id(example_id)
    
    # Example: Check for non-existent entry
    print(f"\nChecking for non-existent entry with id 999...")
    entry_exists(999)
    
    # Example: Try to get non-existent entry  
    print(f"\nTrying to get non-existent entry with id 999...")
    get_entry_by_id(999)
    
    # You can also add more entries like this:
    # add_entry(2, {"message": "hello world"}, 200)
    # add_entry(3, {"items": ["a", "b", "c"]}, 404)]